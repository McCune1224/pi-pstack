...
- [ ] Arm the 30-minute audit tick as a monitored-shell sleep loop with a wake sentinel. Never leave the cadence to memory.
...
Each live lane runs at the PR head. Drive through `control-ui` or `control-cli` (both ship with this plugin).
...
**Verify, live.** Tests alone are not sufficient verification. A PR is verified only when its unit, live, and perf boxes are all checked. Ten parallel lanes at the PR head, per the boot recipe.
