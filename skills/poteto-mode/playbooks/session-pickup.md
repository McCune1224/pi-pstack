...
**You own the resume point. Read the prior trail, don't redo it.** For "take over this", "resume this conversation", "continue from <transcript path>", "you're taking over", "pick up where X left off", a background-run handoff, or a pushed branch you're meant to continue.
...
1. Locate the prior trail. A local Pi session under `~/.pi/agent/sessions/`, one subdirectory per working directory (use the active cwd's subdir; do not glob across other working-directory subdirs, that crosses workspaces and reads unrelated private sessions), a background-run handoff, or a pushed branch. Read the metadata overview and last messages first, then scan back for the decision points. Parse a long transcript in a subagent and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
