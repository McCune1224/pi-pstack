...
3. Pick the runners. Use the `arena runners` role list from Pi settings (`subagents.agentOverrides`, set via `/pstack-setup`) when present. Otherwise default to one runner on each of `inherit`, `inherit`, `inherit`, `inherit`. Spawn more when the arena covers multiple design directions. Same model N times when the work is generation-bound rather than judgment-sensitive.
