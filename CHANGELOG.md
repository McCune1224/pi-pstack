...
- Commands: `/pstack-setup` (model wizard with scope, tier, and per-role fast paths), `/pstack-status` (parent model, resolved mapping, loaded skills), `/bro` (plain-language restatement).
...
- Maintenance levers: `scripts/sync-upstream.sh`, `scripts/check-pi-isms.sh`.